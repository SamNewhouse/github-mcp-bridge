...
describe("getFileContents", () => {
  ...
  it("truncates content and sets truncated: true when file exceeds 3.5 MB", async () => {
    // Build a string just over 3.5 MB
...
describe("getMultipleFiles", () => {
  ...
  it("deduplicates paths before fetching", async () => {
    ...
    // Only 2 unique — mock 2 responses
    mockGithubRequest
    ...
    // Should only have called GitHub twice, not four times
  ...
  it("stops early and adjusts nextCursor when byte budget would be exceeded mid-page", async () => {
    ...
    // Second file: just over the budget
    ...
    // Only the first file should be returned
    expect(result.files).toHaveLength(1);
    expect(result.pagination.hasMore).toBe(true);
    // nextCursor should point at src/b.ts (index 1)
