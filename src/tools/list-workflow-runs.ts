...
export const listWorkflowRunsTool = defineTool({
  ...
  description: "List workflow runs for a repository. Optionally filter by branch, event, or status.",
  input: listWorkflowRunsInputSchema,
  handler: async ({ owner, repo, branch, event, status, perPage }) => ({
    workflow_runs: await listWorkflowRuns(owner, repo, branch, event, status, perPage),
  }),
});
